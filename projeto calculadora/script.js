...
const allowedKeys = [
  ...
  "%",
  " ",
]; // Aqui coloquei todos os simbolos/funções que existem na calculadora, que estão la no HTML

document.querySelectorAll(".charKey").forEach(function (charKeyBtn) {
  // aqui para fazer funcionar quando clica nos botoes da calculadora com o mouse, para cada click executa a function abaixo
...
document.getElementById("clear").addEventListener("click", function () {
  // aqui definido para quando clicar no botao de limpar/apagar o que esta digitado
...
document
  ...
  .addEventListener("click", function (ev) {
    ...
    if (button.innerText === "Copiar resultado") {
      button.innerText = "Concluido!"; //texto q aparecera dps q clica em copias
      ...
      button.innerText = "Copiar resultado"; // aqui para quando clicar dnv pode copiar dnv
...
input.addEventListener("keydown", function (ev) {
  //aqui estou dizendo que quando uma tecla for precionada na calculadora ela vai recber esta funcionabilidade
  ...
  if (allowedKeys.includes(ev.key)) {
    // aqui é se a tecla estiver inclusa dentro das que estao registradas no array
    input.value += ev.key; //aqui se ela existir no array ela aparecec no campo de digitaçao
    return;
  }
  if (ev.key === "Backspace") {
    //aqui se a tecla que o usuario pressionar for igual a de apagar(backspace em ingles)
    input.value = input.value.slice(0, -1); // ele ira remover o ultimo caracter digitado
  }
  if (ev.key === "Enter") {
    // aqui pra quando o usuario apertar a tecla enter
